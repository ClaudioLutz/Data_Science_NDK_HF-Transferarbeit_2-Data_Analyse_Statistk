...
covid_middle <- as.Date("2020-07-01")  # Mid-point of the COVID period for text placement
...
ggplot(monthly_counts, aes(x = as.Date(paste0(year_month, "-01")))) +
  ...
  geom_line(aes(y = scaled_difference, color = "Difference"), size = 1.2) +  # Line for Netto Zuwachs
  ...
  annotate("text", x = covid_middle, y = Inf, label = "Covid", vjust = 2, size = 5, color = "black") +  # Text annotation
  theme_minimal() +
...
# Check if the first month has a partial entry (data does not start on the 1st day)
first_data_date <- min(shab_data$date)
if (day(first_data_date) != 1) {
  # Move the start to the first day of the next month if the first month is partial
  first_full_month <- floor_date(first_data_date + months(1), "month")
}

# Filter the data to include only full months
shab_data_filtered <- shab_data %>%
  filter(date >= first_full_month & date <= last_full_month)

# Create a quarter-year column
shab_data_filtered$quarter_year <- floor_date(shab_data_filtered$date, "quarter")

# Summarize the data for the line plot
shab_data_summary <- shab_data_filtered %>%
